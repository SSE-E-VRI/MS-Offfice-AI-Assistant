...
| **D-5** | Low | **Partially resolved (Phase C2).** 3 of 4 allow-list sites unified to `ToolRegistry` (`PromptAssembler.cs`, `SpreadsheetActionParser.cs`, `PowerPointActionParser.cs`). Controller execution dispatch remains typed pending unified C4 `Execute*` cutover. |
