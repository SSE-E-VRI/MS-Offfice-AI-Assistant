...
in `Connect.cs:322-340`). Four groups, 15 top-level controls (14 buttons + 1 menu) — 23 buttons
total once the Translate menu's 9 nested language items are counted, not 24 top-level as an earlier
revision of this line claimed (adversarial-review finding) — all handled in
...
| Attachments | `AttachmentExtractor` — OpenXML, PdfPig | **Fixed by A4 for .docx/.xlsx/.pptx**: PDF keeps real page numbers; `.docx` paragraphs tagged `[¶N]`; `.xlsx` cells tagged with real address AND real sheet name (resolved from `xl/workbook.xml` via rels). `.pptx` now emits per-slide `[Title]`, `[Section]` (via `presentation.xml` + rels), `[Layout]`, `--- Slide N ---` in correct presentation order (not lexical), speaker notes, and `--- Sections ---` provenance — recognized as citation pattern and more informative than ordinal alone. |
...
D-13 (see §3's defect log) is fully closed: every mutation path reports a structured result. 80
internal `catch { }` sites remain as of this revision (Word 18 / Excel 23 / PowerPoint 39 — **do not
trust this number, recount it**: `grep -cE "catch \{ ?\}$|catch$" src/Hosts/{Word,Excel,PowerPoint}Controller.cs`),
intentionally — every one is a non-mutating read/probe absorbing Office 2010↔365 API differences. The
5 sites (6 call sites) that used to swallow a best-effort mutation sub-step nested inside an already
Tier-1/2-wrapped method were fixed by D-14 and D-16 (now log via `Logger.Warn` on failure instead of
swallowing silently); none remain. This
remains a deliberate, declined-not-deferred decision (see D-13's SSOT entry for the exact reasoning),
not an oversight.
...
`tests/Program.cs` runs 37 suites (as of the last verified commit — this number grows with every
...
`WordMarkdownRenderer.PlanListBlocks` (the Markdig-AST list traversal, split out from the
COM-bound rendering precisely so the "nothing in a list item is dropped" guarantee is testable
here -- see D-18).

Note that this Office-free rule is why `WordMarkdownRendererTests` covers the renderer's
*traversal* only: a test that called `WordMarkdownRenderer.Render` -- even to assert the
null-app no-op -- would drag `NetOffice.WordApi` into the tests csproj and break the rule.
The tests csproj does reference `Markdig`, which is COM-free, plus `tests/app.config` binding
redirects: Markdig's net452 build wants older `System.Memory`/`System.Buffers` than this
solution ships, and unlike an Office host process the standalone runner has nothing to resolve
that for it.
...
| Word: insert, replace, Track Changes, accept/reject, Markdown→table | Implemented (tracked inserts suppress `Document.TrackFormatting` while `WordMarkdownRenderer` runs — it sets font/size/style per emitted range, and Word logged each as a separate "Formatted" revision, burying the one insertion under dozens of rows in the review pane; `TrackRevisions` stays on, so the insert is still a single reviewable Inserted block) |
| Excel: 48 confirm-before-apply action types with safety bounds (ToolRegistry.cs:224 + ExcelController.cs:362) — was 13, see commit history | Implemented |
| PowerPoint: deck build, slide move, sections, speaker notes, image insert | Implemented |
| Embedded offline User Manual | Implemented |
| Grammar-check / edit prompts no longer insert a critique report | Implemented (root cause: asking to grammar-check selected text could get back a multi-section critique - a revised draft, a comparison table, a 'Key Improvements' rundown, a second 'recommended' draft - and Insert applied the ENTIRE thing, table included, as the replacement. Two-part fix. (1) `PromptAssembler.BuildHostAwareSystemPrompt`'s Word branch now tells the model an edit/rewrite/grammar-check response on selected text must be ONLY the finished replacement text - no comparison table, no alternate drafts, no 'Key Improvements'/'Final Recommendation' headings (golden-master hash bumped accordingly, see the bump-reason comment on `ExpectedGoldenMasterSha256`). (2) Defense in depth for when a model ignores that anyway: `ResponseContentCleaner.LooksLikeEditAnalysisReport` flags a comparison-table header row or 2+ analysis-report headings; `ConfirmInsert`'s dialog now shows a plain-text preview (was raw Markdown syntax via `MarkdownClipboard.ToPlainText`) with a warning banner when flagged, and `ResponsePreviewWindow` shows the same warning. Deliberately flags rather than trims - guessing which paragraph is 'the real draft' out of a report shape risks silently dropping the user's actual content) |
| Insert replaces the exact text a Selection-scope prompt was based on | Implemented (`WordController.CreateSelectionBookmark` / `TrySelectSourceBookmark` / `ForgetSourceBookmark`, wired through `ChatMessage.SourceSelectionBookmark`. Previously Insert always re-read Word's *live* selection, so if the user clicked or scrolled elsewhere while a response was streaming in - or just took a few seconds to read it - Insert would replace the wrong text, or the current cursor position, instead of what was actually sent to the model. Now, a Selection-scope prompt pins the exact range with a hidden bookmark the moment it's sent; Insert restores that range before applying, in both the Track Changes and plain paths, then forgets the bookmark. Falls back to the previous live-selection behavior if the pinned text was itself edited or deleted in the meantime. Bookmark creation is capped at 30 tracked entries per session to bound worst-case accumulation from prompts the user never inserts. Covered by `OfficeHostControllerTests.TestSourceSelectionBookmarkHeadlessSafety`) |
| Preview shows what Insert will apply | Implemented (`UI/ResponsePreviewWindow`, a code-built window replacing the raw `MessageBox.Show(msg.Content)` that displayed literal `**` markers and the model's trailing critique of its own draft. Renders the cleaned content through the same `MarkdownHelper` the chat pane uses, with a `Show raw response` toggle, Copy, and modeless keyboard interop. `ResponseContentCleaner`'s break-guarded cue list was widened for this - `key features`, `key points`, `highlights`, `why this works`, `how to use`, `usage / formatting / template notes` - and only the LAST thematic break is the cut point, so a letter's own rule before an `Encl.` block survives. Covered by `XamlLoadTests.TestResponsePreviewWindowShowsCleanedContent`) |
| Action Log renders entries readably | Implemented (`UI/Converters/AuditDisplayConverters` + `ActionHistoryWindow.xaml`. Three faults: the ListBox defaulted to `ScrollViewer.HorizontalScrollBarVisibility=Auto`, which measures each item at its full desired width rather than the viewport's, so the cards' `TextWrapping=Wrap` never engaged and the Undoable column was pushed off-screen - now Disabled with stretched item containers; entries store the raw applied response, so Markdown markers and pipe-tables rendered verbatim - now passed through a plain-text converter; and the full 2000-char entry filled the card - now a 3-line preview with the rest behind a `Full content` expander. `Undoable` also renders as a label instead of a bare `True`. Covered by `XamlLoadTests.TestActionHistoryWindowWrapsLongEntries`) |
| Copy puts formatted content on the clipboard | Implemented (`UI/Helpers/MarkdownClipboard` replaces the old `Clipboard.SetText(raw)`, which pasted the chat wrapper and literal `**` markers into Word. It writes CF_HTML - pasted as real bold / headings / lists / tables - plus a marker-stripped plain-text fallback that keeps bullets and blank lines, both run through `ResponseContentCleaner` so Copy and Insert agree. CF_HTML offsets are byte positions, so the fragment is escaped to numeric entities and kept pure ASCII; a single curly quote otherwise shifts every offset and Word pastes truncated markup. Falls back to plain text if the rich copy throws) |
| Insert strips the AI's conversational wrapper | Implemented (`Core/ResponseContentCleaner` removes the "Here's a polished draft:" lead-in and the trailing "Notes for Customization" / "Let me know if..." block before Word, Excel or PowerPoint insertion — those were landing inside finished documents. Conservative by design: it acts only on a recognised opener, or a trailer marked by a thematic break or an unmistakable heading in the closing half, and returns the original text if it would empty the content. Copy and Preview still show the raw response. Disable with `ConfigManager.StripConversationalWrapper = false`) |
...
| Chat / Plan / Edit modes | Implemented (Phase A1: SessionMode gate hard-blocks the OfficeAction-card approval path in Chat mode, and — since an adversarial-review fix — the Insert button and Accept/Reject Tracked Revisions too, which originally bypassed the gate entirely and could mutate the document even in Chat mode; Undo is deliberately left ungated, see the fix commit for why. Plan mode is now real, not a no-op alias of Edit: `AssistantSession.ProcessAssistantResponse` builds a `Planner`-produced `Plan` instead of populating `OfficeActions`, rendered as an editable, executable `PlanTemplate` card — reorder/skip/remove steps, per-step approve, run, rollback, wired to the already-tested `PlanExecutor`. Single-host only; `CrossHostPlanCoordinator` is not wired into chat, since a single chat session's `ActionExtractor` is already host-scoped) |
...
| ~~D-13~~ | — | **RESOLVED — FULLY CLOSED, no follow-up tracked.** Every UI-reachable mutation method across `WordController`/`ExcelController`/`PowerPointController` now has an `Execute*` wrapper returning `HostOperationResult` (Tier 1, Phase C4: `ExecuteMoveSlide`/`ExecuteCreateSectionBeforeSlide`/`ExecuteRenameSectionInPlace`/`ExecuteSetSpeakerNotesInPlace`; Tier 2: `ExecuteAcceptRevisionsInSelection`/`ExecuteRejectRevisionsInSelection`/`ExecuteUndoLastChange` on Word, `ExecuteUndoLastAction`/`ExecuteInsertText` on Excel, `ExecuteUndo`/`ExecuteInsertText` on PowerPoint), each covered by headless tests in `HostOperationResultTests.cs`. Also removed 8 additional dead methods found during the sweep beyond D-10's original scope (`WordController.ApplyTrackChanges`/`ConvertSelectedMarkdownTableToWordTable`/`InsertMarkdownTableAtCursor`/`ConvertSelectedTextToWordTable`, `ExcelController.ApplySpreadsheetAction`, `PowerPointController.ApplyPowerPointAction`/`ExecutePowerPointAction`/`UndoLastChange`). The original blocking criterion — "Phase C5 verification cannot ship while any UI-reachable mutation swallows" — is satisfied: Phase C5 shipped and every mutation entry point now reports a structured result. **"Tier 3" (converting the internal read/probe `catch { }` sites to the `SafeOfficeProbe<T>` pattern from D-12) was evaluated and explicitly declined, not deferred:** the exact count drifts as the codebase grows and as individual sites get named `catch (Exception ex)` clauses (which drop out of the bare-catch grep below) — **never trust a number quoted in this document; always recount** via `grep -cE "catch \{ ?\}$|catch$" src/Hosts/{Word,Excel,PowerPoint}Controller.cs`. For rough history only: ~67 (Word 10/Excel 18/PowerPoint 39) when this decision was first made; 86 (Word 18/Excel 24/PowerPoint 44) immediately before the D-14 fix; 82 (Word 18/Excel 23/PowerPoint 41) immediately after D-14; 80 (Word 18/Excel 23/PowerPoint 39) after D-16's two further bare-catch fixes (below). **Correction (adversarial review):** the original text here claimed every one of those sites is a non-mutating probe; that was wrong twice over — D-14 found three sites (`PowerPointController.GetActivePresentationCore`'s `Presentations.Add(-1)`, `GetOrCreateActiveSlide`'s two `Slides.Add(...)` fallbacks, `ExcelController.ApplyConditionalFormatting`'s `condition.Interior.Color = ...`), and a later re-review found two more in `PowerPointController.AddSlideUsingPresentationLayout` — its custom-layout `Slides.AddSlide(insertIndex, layout)` attempt and its master-custom-layout `Slides.AddSlide(insertIndex, layouts[layoutIndex])` fallback attempt were each wrapped in a bare `catch { }` that silently discarded a real mutation failure before falling through to the next fallback. **All five (six call sites) now log via `Logger.Warn` on failure instead of swallowing silently** (D-14 fixed the first three, D-16 fixed `AddSlideUsingPresentationLayout`'s two); this did not reopen D-13 as a safety bug in either case because all five sit *inside* a method Tier 1/2 already wraps with a `HostOperationResult` — a failure there still surfaces at the outer `Execute*` boundary, it just previously had no diagnostic trail pointing at which inner step failed. The remaining ~80 sites are genuinely non-mutating probes (COM cross-version property reads, best-effort range/target resolution) — no correctness, safety, or blocking criterion depends on converting them, and the only benefit would be a named diagnostic log line if one of these already-degraded-gracefully reads happens to fail. Decision: not worth ~80 mechanical edits across 3 controller files for a logging-only improvement to code paths that already fail safe. This remains a closed decision on the probe sites; the mutation-swallowing sites found so far are fixed, not merely re-justified — but given two rounds of adversarial review each found sites the previous round missed, **do not assume the audit is exhaustive**; a future pass finding another mutating swallow inside an already-wrapped method should fix it the same way (add a named catch + `Logger.Warn`) without reopening this Tier-3 probe-conversion decision. |
| Note (Phase A forward guidance) | — | `src/UI/ChatSidebar.xaml.cs` calls several legacy controller methods directly (e.g. `_wordCtrl.InsertTextAtCursor`, `_excelCtrl.UndoLastAction`, `_pptCtrl.Undo`) instead of the `Execute*`/`HostOperationResult` wrappers above. Left as-is deliberately — this UI surface is being redesigned by Phase A (mode selector, response cards, approval flow), so rewiring it now would likely be partially thrown away. Phase A's action-approval UI should route through `Execute*` wrappers, not legacy methods. **Update (adversarial-review fix):** `ApplyRevisionDecision` (Accept/Reject Tracked Revisions) was rewired onto `ExecuteAcceptRevisionsInSelection`/`ExecuteRejectRevisionsInSelection`/`ExecuteAcceptAllRevisions`/`ExecuteRejectAllRevisions` — those four D-13 Tier-2 wrappers had zero product callers before this fix (test-only coverage), now they're the live path. **Update 2 (D-15):** `BtnInsertMessage_Click` was also rewired — its Word non-tracked branch now calls `ExecuteInsertText`, Excel calls `ExecuteInsertText`, and PowerPoint calls `ExecuteCreateDeckFromOutline`, all `HostOperationResult`-returning, all checked before recording an audit entry (see D-15). Its Word tracked-changes branch (`ReplaceSelectionWithTrackChanges`/`InsertTextAtCursorWithTrackChanges`) already returned `bool`; that return value is now captured instead of discarded. `BtnUndoChange_Click`'s `_wordCtrl.UndoLastChange()`/`_excelCtrl.UndoLastAction()`/`_pptCtrl.Undo()`/`_hostController.Undo()` remain deliberately unrewired — Undo is intentionally ungated by session mode (§3's Chat/Plan/Edit row) so folding it into the same gated `Execute*` call path as a mutation would need its own design pass, not a mechanical swap. |
| ~~D-14~~ | — | **RESOLVED.** Second adversarial-review pass (two independent reviewers) found: (1) a defense-in-depth ordering gap — `ChatSidebar.ExecuteOfficeAction` gated on Chat mode using `action.RiskLevel` *before* `ActionVerifier.PreVerify` synced that field from `ToolRegistry` (§2.7's single source of truth), so a locally-forged/stale low `RiskLevel` could bypass the Chat-mode gate; not exploitable in practice (extraction-time construction already syncs risk, and a MessageBox approval always intervened before execution) but a real ordering bug. **Fixed:** `PreVerify` now runs before the mode gate, and the batch-preview path (`BtnApplyAllOfficeActions_Click`) also PreVerifies each action before building the warning preview, so the non-undoable badge matches what will actually execute. Regression test: `AssistantSessionModeTests.TestChatModeBlocksAfterPreVerifySyncsRiskLevel`. (2) A 12-member dead-code cluster with zero callers anywhere (product and tests) — `WordController.ExecuteReplaceSelection`/`ExecuteInsertWithTrackChanges`, the `WordMarkdownRenderer` Markdown-table-detection cluster (`ContainsMarkdownTable`/`RenderFirstMarkdownTable`/`RenderDelimitedTable` plus their now-orphaned private helpers `ParseMarkdown`/`FindFirstTable`/`PrepareInsertionRange`/`MoveSelectionToRangeEnd`/`RenderStringTable`), `TokenCounter.CountMessageTokens`, `ConfigManager.GetSettingsForProvider`, `VersionDetector.SupportsFeature` (+ its now-unused `OfficeFeature` enum), and `ChatRequest.NotifyActionsChanged`/`NotifyPowerPointActionsChanged` — all deleted. **Correction (later re-review):** this entry originally also claimed `TryParseDelimitedTable`/`SplitPipeDelimitedRow` were kept because they had live callers "through `WordController.cs:204,213`" — that was wrong: those two line numbers are `ReplaceSelectionWithTrackChanges`/`InsertTextAtCursorWithTrackChanges`, which call `RenderWithTrackChanges` → `WordMarkdownRenderer.Render` (the Markdig-AST path), never `TryParseDelimitedTable`. `TryParseDelimitedTable`/`SplitPipeDelimitedRow` had zero callers anywhere and were deleted in the same follow-up pass (D-15) that caught this. `RenderTable(Word.Range,...)` and the rest of `WordMarkdownRenderer` remain live through `Render`/`RenderBlock`. (3) A .xlsx attachment citation gap — `AttachmentExtractor.ExtractXlsx` emitted bare `Address=Value` cell tags with no sheet qualifier, so a citation click on a non-active sheet's cell in a multi-sheet workbook could resolve to the identically-named cell on whichever sheet happened to be active. **Fixed:** cells are now emitted as `SheetName!Address=Value`, which the existing Pattern-3 citation regex (`MarkdownHelper`, `EvidenceLevel`, `ChatSidebar.NavigateToCitation`'s `excelSheetMatch`) already matched — no matcher/navigation changes needed, only emission. Covered by `AttachmentExtractorProvenanceTests.TestXlsxRealSheetNamesAndCellAddresses`, extended to assert the exact `Sheet!Cell=Value` string and round-trip it through the real `EvidenceLevelClassifier.Classify`. Verified: clean rebuild of `src` and `tests` for both `x86`/`x64`, 33/33 suites × 3 runs × 2 platforms. |
| ~~D-15~~ | — | **RESOLVED.** Third-pass adversarial review (4 reviewers) found: (1) **Live audit-integrity bug** — `BtnInsertMessage_Click` (`ChatSidebar.xaml.cs`) discarded the `bool`/`HostOperationResult` returned by every insert call (Word tracked-changes `ReplaceSelectionWithTrackChanges`/`InsertTextAtCursorWithTrackChanges`, the void `_excelCtrl.InsertText`/`_pptCtrl.CreateOrUpdateDeckFromOutline` wrappers) and unconditionally called `ActionAuditStore.Record(...)` with a hardcoded success-shaped `ApplyResult` string (e.g. `"Inserted into Excel"`) immediately after — so a failed insert (protected worksheet, no accessible `ActiveDocument`, PowerPoint deck-build failure) was recorded in the audit trail identically to a real success, contradicting §2.9's "written only after success" and D-13's "every mutation entry point reports a structured result." **Fixed:** the Word non-tracked branch now calls `WordController.ExecuteInsertText` (which explicitly fails when `GetApp()` returns null, unlike the raw void `InsertTextAtCursor` it wraps, whose null-app path silently no-ops with no signal at all), Excel calls `ExecuteInsertText`, PowerPoint calls the pre-existing `ExecuteCreateDeckFromOutline` (previously unused by this call site — it already existed, wrapping the same `InsertText` the void `CreateOrUpdateDeckFromOutline` also wraps, just without discarding the result), and the Word tracked-changes branch now captures the `bool` it always returned instead of ignoring it. `ActionAuditStore.Record` is now called only on success; failure shows a `MessageBox` with the real error text instead of a silently-wrong audit entry. (2) **D-14 factual error, corrected:** D-14 claimed `WordController.cs:204,213` were live callers of `WordMarkdownRenderer.TryParseDelimitedTable`/`SplitPipeDelimitedRow` — those line numbers are actually `ReplaceSelectionWithTrackChanges`/`InsertTextAtCursorWithTrackChanges`, which never call either method (they go through `RenderWithTrackChanges` → the Markdig-AST `Render` path). Both methods had zero callers anywhere and are now deleted. (3) **Bare-catch count correction:** D-14's own fix (4 call sites converted from bare `catch{}`/`catch` to named `catch (Exception ex)` with `Logger.Warn`) dropped the grep-counted total from 86 to 82 (Word 18/Excel 23/PowerPoint 41) — §2.12 and D-13 previously still quoted 86 as "current as of this revision" immediately after the count had changed; corrected, and reworded to stop asserting freshness that goes stale on the next commit. `ToolRegistry.GetAllTools()` (zero callers, flagged by the same review pass) was assessed and kept — it is a one-line, self-contained registry-enumeration counterpart to the already-used `GetToolsForHost`, not a parallel/speculative feature, so the risk/benefit of deleting it doesn't clear the bar the rest of this cleanup did. Verified: clean rebuild of `src` for both `x86`/`x64` after the `BtnInsertMessage_Click` fix. |
| ~~D-16~~ | — | **RESOLVED.** Fourth-pass adversarial review (multiple reviewers, one read-only/static-only) found: (1) **D-15 orphan cascade** — deleting the void `PowerPointController.CreateOrUpdateDeckFromOutline` call site inside `BtnInsertMessage_Click` (D-15) left the method itself dead (its only remaining caller was `DocToDeckTests.cs:108`, a headless smoke call with no assertion on the result). **Fixed:** the test now calls `ExecuteCreateDeckFromOutline` instead and asserts `!Success` for the headless (no-app) case — a stricter test than before, not just a rename — and the dead `CreateOrUpdateDeckFromOutline` is deleted. `WordController.ReplaceSelection` was similarly orphaned (its only caller, `ExecuteReplaceSelection`, was deleted by D-14) and is also deleted; its only internal dependency, `DeleteCurrentSelection`, keeps a live caller in `RenderWithTrackChanges`. (2) **Stale SSOT doc-to-deck row** — §"Next slice" still named the now-deleted `CreateOrUpdateDeckFromOutline:368` as the deck-apply entry point (and that line number was already wrong before this pass); corrected to `ExecuteCreateDeckFromOutline:455`. (3) **Two more mutation-swallowing bare catches**, same class as D-14's: `PowerPointController.AddSlideUsingPresentationLayout`'s custom-layout `Slides.AddSlide(...)` attempt and its master-custom-layout `Slides.AddSlide(...)` fallback attempt each silently discarded a real `AddSlide` mutation failure via a bare `catch { }` before falling through to the next fallback (ultimately `Slides.Add(insertIndex, 2)`), contradicting §2.12/D-13's "every remaining bare catch is a non-mutating probe" claim. **Fixed:** both now log via `Logger.Warn` before falling through; behavior (which layout ends up used) is unchanged, only the diagnostic trail is added. (4) **Bare-catch count re-drifted, corrected again**: fixing (3) dropped the grep-counted total from 82 to 80 (Word 18/Excel 23/PowerPoint 39); §2.12 and D-13 updated, with an explicit note added that repeated rounds of adversarial review have each found bare-catch sites the previous round missed, so this decision's probe-site assessment should not be treated as exhaustively re-verified just because two passes now agree — only that no *currently known* site swallows a mutation silently. Verified: clean rebuild of `src` and `tests` for both `x86`/`x64`, 33/33 suites × 3 runs × 2 platforms. |
| ~~D-17~~ | — | **RESOLVED.** Adversarial review of `installer/setup-x64.iss` / `setup-x86.iss` found two compile-fatal Inno Setup bugs, identical in both files: **(1)** `[Registry]` `Subkey` values embedding the task-pane CLSID/category GUIDs as literal `{9B3C7624-...}` braces — Inno Setup's `{constant}` substitution applies inside any quoted string in a section entry, not just `[Setup]` key=value lines, so an unescaped `{` there is parsed as the start of a constant reference; since `9B3C7624-...` etc. are not known constants, the compiler fails with "Unknown constant." (`AppId={{2F8D4B61-...}` two lines above already did this correctly — the 5 `CLSID`/`Implemented Categories` `Subkey` lines below it, each with 1–2 embedded GUIDs, did not.) **Fixed:** every embedded GUID's opening brace doubled to `{{`, matching the `AppId` line's existing pattern. **(2)** `InitializeSetup`'s `ShellExec(..., ewNoWait, 0)` passed the literal `0` for the function's final parameter, which Pascal Script declares `var ResultCode: Integer` (an out-parameter, not an input) — a literal is not a valid argument for a `var` parameter, so the compiler fails with "Variable expected." **Fixed:** declared a local `ResultCode: Integer` and passed it by reference. Both bugs were compile-time-fatal (the installer scripts could not produce a `.exe` at all, in either bitness) yet were previously undetected because `install.cmd`/`install.ps1` never invoke Inno Setup — only `installer/setup-*.iss` does, and nothing in the standalone test suite (COM-free by design, §2.14) touches it. **Verified statically** (no stray unescaped `{GUID` pattern remains via `grep -nE "[^{]\{[0-9A-Fa-f]{8}-"`; no `ewNoWait, 0)` calls remain) — **not** compiled, because this machine has no Inno Setup / `ISCC.exe` installed; compiling and producing both `.exe` outputs remains an open verification step for whoever next has Inno Setup 6 available. |
| ~~D-18~~ | — | **RESOLVED.** A Markdown table nested inside a list item was silently discarded and never reached the document. `WordMarkdownRenderer.RenderList` walked each `ListItemBlock`'s children through an `if (ParagraphBlock) ... else if (ListBlock) ...` chain with **no final else**, so any other block type a list item can hold -- a `Table`, a `FencedCodeBlock`, a `QuoteBlock`, a nested `HeadingBlock` -- fell off the end of the dispatch and was dropped. `RenderBlock` had always handled `Table` correctly; the table simply never got there. This is a common real-world shape: models routinely put a spec table under a numbered point (`2. **Request:** ... | Parameter | Specification |`), and Markdig parses that table as a child of the `ListItemBlock`, not as a top-level sibling. Symptom seen in the field: the AI Response Preview showed the table (it renders the raw Markdown), the inserted Word document jumped straight from the "Request:" paragraph to "Assurance:", and nothing was logged because the drop was a missing branch rather than a caught exception. **Fixed:** the list-item traversal is split into `PlanListBlocks(ListBlock)` -- a pure, COM-free method returning every block every list item contains, in document order -- which `RenderList` consumes; paragraphs still become numbered/bulleted lines, and **every** other block is delegated to `RenderBlock`, so dropping is no longer representable in the control flow. Four supporting fixes in the same path: **(a)** list numbering/indent is now cleared off the insertion point (`ClearListFormat`) before a nested block starts, and again defensively inside `RenderTable`, or Word builds a numbered, indented table out of the list format it inherited; **(b)** `RenderTable`'s `bool` return was computed but ignored by `RenderBlock` -- a table Word refuses to create (`Tables.Add` throws) took the data down with it, leaving only a `Logger.Warn`; the return is now honoured and falls back to `RenderTableAsText` (one paragraph per row, tab-separated cells) so the content still reaches the document; **(c)** the delimiter row's column alignment (`:---`, `:---:`, `---:`) is now applied to the Word cells via `ApplyColumnAlignment`. Note that Markdig 0.22.0 emits **one more** `ColumnDefinitions` entry than the table has real columns (4 defs for a 3-column table), so the Word table must continue to be sized from the widest `TableRow` and the alignment lookup is bounds-checked against the real column count rather than trusting `ColumnDefinitions.Count`; **(d)** `RenderTable` unconditionally called `InsertParagraphAfter()` after the table, but `Tables.Add` already leaves the paragraph the range sat in just past the table's end -- that left a stray blank paragraph under every table. It now inserts one only when `IsInsideTable` (`Range.Information(wdWithInTable)`) says the range is still within the table, and fails safe by assuming it is when the check itself throws, so following content can never land inside a cell. Regression test: `tests/WordMarkdownRendererTests.cs` asserts, over the exact letter shape from the field report, that the nested table (and a nested fenced code block and quote) are planned for rendering, that the list's own paragraphs still are, and that Markdig's spurious trailing column definition and its alignment parsing both still behave as the renderer assumes. Per §2.14 the suite stays Office-free, so the test covers `PlanListBlocks` rather than `Render` itself -- calling `Render` would pull `NetOffice.WordApi` into the tests csproj. Adding `Markdig` to that csproj also required `tests/app.config` binding redirects (Markdig's net452 build references older `System.Memory`/`System.Buffers`/`System.Numerics.Vectors`/`System.Runtime.CompilerServices.Unsafe` than the solution ships; an Office host process resolves these, the standalone runner does not). **Verified against a live Word instance** (Office x64) as well as by clean rebuild of `src` for both `x86`/`x64` Release and of `tests`, 37/37 suites passing. The live check drove `WordMarkdownRenderer.Render` against real documents and dumped the resulting object model: the table lands as a real 3x2 Word table with the right cells, the delimiter row's alignments reach the cells, numbering runs 1./2./3. straight across the item that holds the table, and no blank paragraph is left under it. |
| ~~D-19~~ | — | **RESOLVED.** Ordered-list numbering was wrong in three separate ways, all in `WordMarkdownRenderer` and all confirmed against a live Word instance by dumping the resulting object model. **(1) Every item restarted at "1."** -- `RenderList` called `ApplyNumberDefault()` on each freshly inserted paragraph as it rendered it. Applying list formatting to a single paragraph makes Word open a *brand new* one-item list each time, so nothing accumulated into a running sequence. **(2) Whole items silently lost their number** -- the `RemoveNumbers` that ended the list walk ran on the trailing empty paragraph, which was still a member of the list Word had just created for the last item, so it stripped the numbering off that entire list. This is why the field report showed unnumbered headings where the chat preview showed "1.", "2.", "3.". **(3) Continuation lines got their own number** -- `RenderInlines` rendered a Markdig `LineBreakInline` as `"
"`, which Word converts to a paragraph mark, splitting one list item into two paragraphs; inside a list the second half picked up the next number, turning a single point into `1. Background:` / `2. The Electrical Division is unable to...`. **Fixed:** line breaks now emit `""` (Chr(11), Word's manual line break) so a soft break stays inside its paragraph; and numbering is applied in one coordinated pass after the whole list *tree* is written, not per item during rendering. `RenderListItems` records each item's span, outline level and ordered-ness as a `ListItemPlacement`, and `ApplyListFormatting` then applies a single list template per kind, with only the first item of each template starting a new list and every later item continuing it at its own outline level. Nested lists recurse *within the same pass* rather than going through `RenderBlock`, because a separate pass leaves Word unable to tell where the outer list resumes -- the outer item after a nested list then continued the inner list's sequence. Ordered lists use `wdOutlineNumberGallery` template 2, which numbers "1." at the top level (matching both a plain one-level list and the chat preview) and "1.1." below it; template 1 ("1)" / "a)") was rejected because it would change the look of every existing flat list, which is the common case. The bullet gallery is single-level, so nested bullets get their indent set *absolutely* from the level -- adding to whatever Word left on the paragraph compounded the previous item's indent (36 / 54 / **90** instead of 36 / 54 / 54). Two related traps worth keeping in mind: marking the first item of a *nested* list as a fresh list start puts that item back at level 1, and `Range.ListFormat` continuation is per list *template*, which is what lets an ordered list resume correctly across a nested bulleted one. **Verified live** across nested ordered lists (1. / 1.1. / 1.2. / 2. / 3.), an item holding a table (1. / 2. / table / 3. / 4.), two separate lists each restarting at 1., nested bullets, and a list item holding a fenced code block and a quote -- plus clean rebuild of `src` for both `x86`/`x64` Release and of `tests`, 37/37 suites passing. Note the standalone COM harness used for the live check needs the same `app.config` binding redirects as the tests csproj (§2.14): without them Markdig's list parser throws `FileLoadException` on `System.Memory` and `Render` silently falls back to `TypeText`, which looks exactly like a rendering bug. The add-in inside Word is unaffected -- the host process resolves those assemblies. A third trap, found the same way: *starting* a list is a level-1 act as far as Word is concerned, so the first item of a template that happens to be nested -- an ordered list opening underneath a bullet -- comes back at the top level however it was applied. `ListLevelNumber` is therefore corrected explicitly after the template is applied whenever it does not match the intended level. Mixed nesting is covered both ways: ordered-outer/bulleted-inner renders 1. / o / o / 2. / 3., and bulleted-outer/ordered-inner renders bullet / 1.1. / 1.2. / bullet. |
...
| Word outline + prompt-relevant context | `WordController.TryGetLiveDocumentOutline` (internal, wraps `WordDocumentContextBuilder.BuildDocumentOutline`), `GetRelevantDocumentContext` — `GetDocumentOutline` never existed under that name and was removed from this table (adversarial-review finding) |
...
| Deck apply | `PowerPointController.ExecuteCreateDeckFromOutline:455` (the void `CreateOrUpdateDeckFromOutline:368` this row previously named was a thin wrapper around the same `InsertText` call, dead-coded by D-15's `BtnInsertMessage_Click` rewire, and deleted by D-16 — see D-16's entry) |
